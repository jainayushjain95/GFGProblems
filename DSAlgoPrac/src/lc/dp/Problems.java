...
class PowerPair {
	...
	public PowerPair(int element, long power) {
		this.element = element;
		this.power = power;
	}
}

class SortArray implements Comparator<PowerPair> {
	public int compare(PowerPair powerPair1, PowerPair powerPair2) {
		if(powerPair1.power > powerPair2.power) {
			return 1;
		} else if(powerPair1.power < powerPair2.power) {
			return -1;
		} else if(powerPair1.element > powerPair2.element) {
			return 1;
		}
		return -1;
	}
...
public class Problems {
	...
	public static void main(String[] args) {
		...
		System.out.println((new Problems().getKth(1, 1000,777)));
	}



	public int[][] maxSumSubmatrixPreCom(int[][] matrix, int k) {
		int[][] dpArray = new int[matrix.length + 1][matrix[0].length];

		for(int i = 1; i < dpArray.length; i++) {
			for(int j = 1; j < dpArray[i].length; j++) {
				dpArray[i][j] = matrix[i - 1][j - 1] + dpArray[i - 1][j] 
	...
	public int getKth(int lo, int hi, int k) {
		PowerPair[] array = new PowerPair[hi - lo + 1];
		dpMap = new HashMap<Integer, Long>();

		for(int i = 0; i < array.length;i++) {
			array[i] = new PowerPair(lo + i, getPower(lo + i));
			dpMap.put(lo + i, array[i].power);
		}
		Arrays.parallelSort(array, new SortArray());

		return array[k - 1].element;
	}

	public long getPower(int element) {
		long power = 0;
		while(element != 1) {
			if(element % 2 == 0) {
				element = element >> 1;
			...
			power++;
			if(dpMap.containsKey(element)) {
				power += dpMap.get(element);
				break;
			}  
		}
	...
	public int sumRegion(int[][] matrix, int row1, int col1, int row2, int col2) {
		int[][] dpArray = new int[matrix.length + 1][matrix[0].length + 1];
		for(int i = 1; i < dpArray.length; i++) {
			for(int j = 1; j < dpArray[i].length; j++) {
				dpArray[i][j] = matrix[i - 1][j - 1] + dpArray[i - 1][j] 
						+ dpArray[i][j - 1] - dpArray[i - 1][j - 1];
	...
	public int minCostClimbingStairs(int[] cost) {
		...
		for(int i = 3;i < dpArray.length; i++) {
			dpArray[i] = Math.min(dpArray[i - 1] + cost[i - 2], dpArray[i - 2] + cost[i - 3]);
		}
		return dpArray[dpArray.length - 1];
	}

	public int minCostClimbingStairsRec(int[] memo, int[] cost, int index) {
		...
		int stepOneCost = ((index == -1) ? 0 : cost[index]) + minCostClimbingStairsRec(memo, cost, index + 1);
		int stepTwoCost = ((index == -1) ? 0 : cost[index]) + minCostClimbingStairsRec(memo, cost, index + 2);
		int minCost = Math.min(stepOneCost, stepTwoCost);
		if(index >= 0) {
			memo[index] = minCost;	
		}
		return minCost;
